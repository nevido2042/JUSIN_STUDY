...
{
	m_tInfo = { 100.f, 400.f, 40.f, 40.f };//초기위치
	m_fSpeed = 10.f;//이동속도
...
void CPlayer::Create_Bullet()
{
	...
	case BUL_NORMAL:
		...
		break;
	case BUL_ELEC:
		CObjMgr::Get_Instance()->Add_Object(OBJ_BULLET, CAbstractFactory<CBullet_Elec>::Create(m_tInfo.fX-40.f, m_tInfo.fY-30.f, m_eDir));
		break;
	case BUL_GUT:
    ...
    break;
	case BUL_ICE: 
		CObjMgr::Get_Instance()->Add_Object(OBJ_BULLET, CAbstractFactory<CIce_Slasher>::Create(m_tInfo.fX, m_tInfo.fY, m_eDir));
		break;
	case BULFIRE:
		CObjMgr::Get_Instance()->Add_Object(OBJ_BULLET, CAbstractFactory<CFire_Player_Bullet>::Create(m_tInfo.fX, m_tInfo.fY, m_eDir));
		break;
	case BUL_CUT:
...
void CPlayer::Key_Input()
{
	...
	if (CKeyMgr::Get_Instance()->Key_Pressing('Z'))
	{
		...
		//m_fJumpPower += 0.5f; // 점프 파워 증가
			...
			m_fJumpPower += 0.7f; // 점프 파워 증가

			if (m_fJumpPower >= 20.f)
				m_fJumpPower = 20.f; // 최대 점프 파워 제한
	}
