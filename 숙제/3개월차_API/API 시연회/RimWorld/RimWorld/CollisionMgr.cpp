...
void CCollisionMgr::Collision_Circle(OBJID _eDst, OBJID _eSrc)
{
	...
	for (auto& pDst : CObjMgr::Get_Instance()->Get_List()[_eDst])
	{
		for (auto& pSrc : CObjMgr::Get_Instance()->Get_List()[_eSrc])
		{
			if (Check_Circle(pDst, pSrc))
			{
				pDst->OnCollision(_eSrc, pSrc);
				pSrc->OnCollision(_eDst, pDst);
			}
		}
	}
}
