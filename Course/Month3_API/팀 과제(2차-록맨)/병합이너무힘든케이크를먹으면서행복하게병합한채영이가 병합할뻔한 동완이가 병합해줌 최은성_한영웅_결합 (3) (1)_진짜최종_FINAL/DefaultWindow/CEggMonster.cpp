...
CEggMonster::CEggMonster(float _x, float _y, float _min, float _max) : m_eState(END), m_fDinstance(0), m_iDirection(1),
m_fMinY(_min), m_fMaxY(_max), m_iCount(_max), m_pBulletList(nullptr)
{
    m_tInfo.fX = _x;
    m_tInfo.fY = _y;
...
{
    float fWidth = m_pTarget->Get_Info().fY - m_tInfo.fY;
