...
{
    _float3 vLook{};
    XMStoreFloat3(&vLook, XMVector3Normalize(pTransform->Get_State(STATE::LOOK)));

    _float3 vUp{};
    XMStoreFloat3(&vUp, XMVector3Normalize(pTransform->Get_State(STATE::UP)));

    _float3 vPos{};
    XMStoreFloat3(&vPos, pTransform->Get_State(STATE::POSITION));

    memcpy(&m_ListenerAttr.position, &vPos, sizeof _float3);
