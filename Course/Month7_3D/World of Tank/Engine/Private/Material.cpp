...
HRESULT CMaterial::Initialize(const _char* pTexPath)
{
    if (nullptr == pTexPath)
        return E_FAIL;
...
#pragma message ("메테리얼 없는 메쉬가 있으면 터짐, 무조건 넣어줘야함")
    ...
    // Diffuse 슬롯 (1번)에 등록
    m_SRVs[aiTextureType_DIFFUSE].push_back(pSRV);
...
CMaterial* CMaterial::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const _char* pTexPath)
{
    CMaterial* pInstance = new CMaterial(pDevice, pContext);

    if (FAILED(pInstance->Initialize(pTexPath)))
    {
