...
CAimCircle::CAimCircle(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CUIObject{ pDevice, pContext }
{

}

CAimCircle::CAimCircle(const CAimCircle& Prototype)
	: CUIObject(Prototype)
{

}

HRESULT CAimCircle::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CAimCircle::Initialize(void* pArg)
{
...
void CAimCircle::Priority_Update(_float fTimeDelta)
{
...
void CAimCircle::Late_Update(_float fTimeDelta)
{
	...
	_vector vWorldPos = XMLoadFloat3(&vGunPickedPos);
...
CAimCircle* CAimCircle::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CAimCircle* pInstance = new CAimCircle(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CAimCircle");
...
CGameObject* CAimCircle::Clone(void* pArg)
{
	CAimCircle* pInstance = new CAimCircle(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CAimCircle");
		Safe_Release(pInstance);
	}
